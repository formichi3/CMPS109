...
//  bool parse::isFact(string name);
  vector<string>compare_params(rule p_rule);
  vector<string>fact_params(string a_fact);
  vector<string>rule_params(rule p_rule);
  template<typename T>
  void make_pfact(rule p_rule);
vector<string> compare_pparams(rule p_rule);
};
